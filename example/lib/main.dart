...
class MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> testReceipt(NetworkPrinter printer) async {
    ...
    // Print image
    final ByteData data = await rootBundle.load('assets/escGO.jpg');
    final Uint8List bytes = data.buffer.asUint8List();
    final Image? image = decodeImage(bytes);
    if (image != null) {
      printer.imageRaster(image, imageFn: PosImageFn.bitImageRaster);//printer.image(image);
    }
  ...
  Future<void> printDemoReceipt(NetworkPrinter printer) async {
    // Print image
    final ByteData data = await rootBundle.load('assets/escudo.jpg');
    final Uint8List bytes = data.buffer.asUint8List();
    final Image? image = decodeImage(bytes);
    if (image != null) {
      printer.imageRaster(image, imageFn: PosImageFn.bitImageRaster);;
    }
